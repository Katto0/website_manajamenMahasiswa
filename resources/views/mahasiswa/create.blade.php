...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4 py-2">
        <a href="{{ route('mahasiswa.index') }}" class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-primary hover:bg-slate-50 transition-colors shadow-sm">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Tambah Mahasiswa Baru</h1>
            <p class="text-sm text-slate-500">Isi data akademik dan pribadi mahasiswa di bawah ini secara lengkap.</p>
        </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 sm:p-8 shadow-sm">
        <form action="{{ route('mahasiswa.store') }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama') }}" placeholder="Contoh: I Made Devano" 
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex gap-6 pt-1">
                            <label class="inline-flex items-center text-sm gap-2 cursor-pointer group">
                                <input type="radio" name="jenis_kelamin" value="Laki-laki" {{ old('jenis_kelamin') === 'Laki-laki' ? 'checked' : '' }} class="w-4.5 h-4.5 text-primary border-slate-350 focus:ring-primary">
                                <span class="text-slate-700 group-hover:text-slate-900 font-medium">Laki-laki</span>
                            </label>
                            <label class="inline-flex items-center text-sm gap-2 cursor-pointer group">
                                <input type="radio" name="jenis_kelamin" value="Perempuan" {{ old('jenis_kelamin') === 'Perempuan' ? 'checked' : '' }} class="w-4.5 h-4.5 text-primary border-slate-350 focus:ring-primary">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="tugas" id="tugas" 
                            ...
                            <option value="Selesai" {{ old('tugas') === 'Selesai' ? 'selected' : '' }}>Selesai</option>
                            <option value="Belum Selesai" {{ old('tugas') === 'Belum Selesai' ? 'selected' : '' }}>Belum Selesai</option>
                        </select>
                        @error('tugas')
                            <p class="text-xs text-rose-650 mt-1 font-medium">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-6">
                    ...
                    <!-- Jurusan -->
                    <div class="space-y-2">
                        <label for="jurusan" class="text-xs font-semibold text-slate-600">Jurusan / Program Studi</label>
                        <input type="text" name="jurusan" id="jurusan" value="{{ old('jurusan') }}" placeholder="Contoh: Teknik Informatika" 
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-slate-150">
                <a href="{{ route('mahasiswa.index') }}" class="w-full sm:w-auto text-center px-5 py-2.5 text-sm font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 active:bg-slate-300 rounded-xl transition-all">
                    Batal
                </a>
                <button type="submit" class="w-full sm:w-auto px-6 py-2.5 text-sm font-semibold text-white bg-primary hover:bg-primary-hover rounded-xl shadow-lg transition-all cursor-pointer">
                    Simpan Mahasiswa
                </button>
            </div>
